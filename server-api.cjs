...
const MODEL_QUOTA_CONFIG = {
  minimax: {
    ...
    billingDay: 16,        // 每月16日到期
    color: '#00e5ff',
    icon: '🟢',
  },
  'volcano-plan': {
    label: '火山方舟 Agent Plan',
    provider_id: 'volcano-plan',
    model_name: 'ark-code-latest',
    monthlyQuotaCalls: 100000,
    billingDay: 20,        // 每月20日到期
    color: '#7c4dff',
    icon: '🟣',
  },
  volcano: {
...
// 从 CC-Switch DB 读取指定 provider 在时间范围内的用量
...
c = conn.cursor()
c.execute('SELECT COUNT(*), COALESCE(SUM(input_tokens),0), COALESCE(SUM(output_tokens),0) FROM proxy_request_logs WHERE provider_id=? AND created_at>=? AND created_at<=?', [${JSON.stringify(providerId)}, ${startTs}, ${endTs}])
calls, inp, out = c.fetchone()
c.execute('SELECT date(created_at,\"unixepoch\") as d, COUNT(*), COALESCE(SUM(input_tokens),0), COALESCE(SUM(output_tokens),0) FROM proxy_request_logs WHERE provider_id=? AND created_at>=? AND created_at<=? GROUP BY d ORDER BY d', [${JSON.stringify(providerId)}, ${startTs}, ${endTs}])
by_day = {r[0]: {'calls':r[1],'input':r[2],'output':r[3]} for r in c.fetchall()}
conn.close()
...
function getModelBalancerConfig() {
  try {
    ...
    const modelParams = cfg?.agents?.defaults?.models || {}
    ...
    const chain = [
      { ...parseModelId(primary), role: 'primary', label: MODEL_QUOTA_CONFIG[parseModelId(primary).provider]?.label || primary },
      ...fallbacks.map(f => ({ ...parseModelId(f), role: 'fallback', label: MODEL_QUOTA_CONFIG[parseModelId(f).provider]?.label || f })),
    ]
    ...
    return {
      ...
      description: '主模型失败/限流时，按顺序自动降级到备用模型',
      params: modelParams,
    }
  } catch {
    return { primary: 'unknown', fallbacks: [], chain: [], strategy: 'unknown', description: '', params: {} }
  }
}
...
app.get('/api/token/quota', async (req, res) => {
  ...
  const providers = Object.entries(MODEL_QUOTA_CONFIG).map(([key, cfg]) => {
    const period = getProviderBillingPeriod(now, cfg.billingDay)
    const usage = getProviderUsageFromCC(cfg.provider_id, period.startTs, period.endTs)
    const totalTokens = usage.input_tokens + usage.output_tokens
    const callsPercent = Math.min(100, Math.round((usage.calls / cfg.monthlyQuotaCalls) * 100))
    const callsRemaining = cfg.monthlyQuotaCalls - usage.calls

    // 判断当前是否为活跃主模型
    const isPrimary = balancer.primary.startsWith(key + '/')
    const isFallback = balancer.fallbacks.some(f => f.startsWith(key + '/'))
    const balancerRole = isPrimary ? 'primary' : isFallback ? 'fallback' : 'standby'

    // MiniMax 实时配额（额外信息）
    let realtimeQuota = null
    if (key === 'minimax') {
      // 异步获取，不阻塞
      realtimeQuota = null // 前端已有 token stats 面板会查
    }
